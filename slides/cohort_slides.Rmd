...
survival_data %>%
  ...
  filter(name %in% c("Seen Shot", "Been Shot")) |>
  ...
  labs(x = "Age", y = "Proportion", color = "Outcome", title = "Exposure to Gun Violence") +
  theme( panel.grid = element_blank(),
...
`Seen Shot` = glm(formula = factor(seen_shot_ever) ~ cohort + sex + race + 
    PCA_disadv_09 + hom_rate_avg_09 + collective_efficacy_09 + pcimgen + pc_relstatus, family = binomial, data = survival_data[["seen_shot"]] %>% filter(w5_sample) |>
      mutate(seen_shot_ever = ifelse(age_right > 27.2, "no", seen_shot_ever))),
`Been Shot` = glm(formula = factor(been_shot_ever) ~ cohort + sex + race + 
    PCA_disadv_09 + hom_rate_avg_09 + collective_efficacy_09 + pcimgen + pc_relstatus, family = binomial, data = survival_data[["been_shot"]] %>% filter(w5_sample) |>
...
map_dfr(logit_models, ~tidy(marginalmeans(., type = "response")), .id = "outcome") %>%
  filter(term == "cohort") %>%
  mutate(outcome = fct_relevel(outcome, "Seen Shot"),
