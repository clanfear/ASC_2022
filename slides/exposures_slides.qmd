...
... but many moved *far away* from Chicago

&nbsp;

. . .

How common is gun violence where respondents live **now**?
...
ggplot(aes(x = race, y = death_or_injury, fill = race)) +
  scale_fill_manual(values = c("NH Black" = "#00BA38", "NH White" = "#F8766D", "Hispanic" = "#619CFF")) +
  geom_col() +
  scale_y_continuous() +
...
gva_radius_nb |>
  ...
  mutate(across(c(estimate, conf.low, conf.high), ~exp(.))) |>
   ggplot(aes(x = estimate, y = term)) + 
    ...
    scale_x_log10(breaks = c(0.2, 1, 5, 20), labels = c("0.2", "1.0", "5", "20")) +
    geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), size =0.5, height = 0.25, color = "white") +
    labs(y = NULL, x = "Rate Ratio", title = NULL, subtitle = NULL) +
    theme_minimal(base_size = 24) + 
...
-   No white respondents shot in adulthood
